...
def plot_graph(graph: nx.Graph, labels, pos = None, **kwargs):
    '''
    plots graph in plt.figure

    '''
    graph = deepcopy(graph)
    saved_indices = []
    for i, (l, t) in enumerate(zip(labels, deepcopy(graph.edges))):
        if l < 1e-2:
            graph.remove_edge(*t)
        else:
            saved_indices.append(i)

    figure = plt.figure(**kwargs) #(figsize = (6, 6), dpi = 200)
    if pos is None:
        pos = nx.spring_layout(graph)      # закомменировано чтобы сохранить расположение вершин

    nx.draw_networkx(graph, pos = pos, node_size = 100, with_labels= True, font_size = 8)
    edge_labels = {t: f"{Decimal(labels[i]):.1e}" for i, t in zip(saved_indices, graph.edges)}
    # edge_labels = {t: f"{Decimal(graph.edges[t]['params']['bandwidth']):.1e}" for t in graph.edges}
    not_print = nx.draw_networkx_edge_labels(graph, pos = pos, edge_labels = edge_labels, font_size = 12, label_pos=0.3)
    return pos